...
export function computeItem({ custoUnit, quantidade, frete, comissaoPct, impostoPct }) {
  const custoTotal = (custoUnit || 0) * (quantidade || 0);
  const base = custoTotal + (frete || 0);
  ...
  const margem = precoVendaTotal - custoTotal - (frete || 0) - impostoValor;
...
export function computeTotals(itens) {
  const totals = itens.reduce(
    ...
    { custoTotal: 0, freteTotal: 0, impostoTotal: 0, precoTotal: 0, margemTotal: 0 }
  );
